#include <xc.h>
#include "tmr0.h" // Include the tmr0.h file
...
next:
    ...
    delay(lcd_delay);
    E = 0;
    temp = (unsigned char)(a << 4); // Cast the result of the shift operation to unsigned char
    temp &= 0xf0;
    LCDPORT &= 0x0f;
    LCDPORT |= temp;
    E = 1;
...
void lcd_init(void)
{
    ...
    lcd_init_write(0x20);
    delay(lcd_delay);
     lcd_com(0x28); // Set 4-bit mode, 2 lines, 5x8 font
    delay(lcd_delay);
    lcd_com(0xC);
    delay(lcd_delay);
    lcd_com(4);
